...
public:
    vector<int> intersect(vector<int>& nums1, vector<int>& nums2) {
        ...
        if (nums1.size() > nums2.size())
            set_intersection(nums1.cbegin(), nums1.cend(), nums2.cbegin(), nums2.cend(), back_inserter(result));
        else
            set_intersection(nums2.cbegin(), nums2.cend(), nums1.cbegin(), nums1.cend(), back_inserter(result));
        return result;
    }
};
...
public:
    vector<int> intersect(vector<int>& nums1, vector<int>& nums2) {
        multiset<int> nums_set1, nums_set2;
        size_t i;
